...
fileID = fopen( strcat(strcat(strcat(root,'/../tid2013/'),prefix),'_lst.txt'), 'r');
...
data_disk=zeros(256,1,1,num_total_samples); 
label_disk=zeros(1,num_total_samples); 

%%
for i=1:1:n(1)
    score = C{1,2}(i); 
    fileID = fopen( strcat('/Users/felisjhou/Dropbox/CITI', C{1,1}{i,1}), 'r');
    feat = textscan(fileID, '%f');   
    data_disk(:,:,:,i)=feat{1}; 
